...
> `0.3.0-SNAPSHOT` 是开发候选版本，尚未发布。在 `0.3.0` 正式公布前，`0.2.0` 仍是当前稳定线。
...
| 版本线 | 状态 | 适用场景 |
|---|---|---|
| `0.2.0` | 当前稳定版 | 现有应用与兼容性参考基线 |
| `0.3.0-SNAPSHOT` | 未发布的开发候选版 | 仅用于仓库开发与发布验证 |
| `0.3.0` | 计划稳定版 | 有序运行时、新 API 与保留的 0.2 兼容面 |
...
| 源码兼容 | 现有 0.2 公开调用可以针对候选版编译 |
| 二进制兼容 | 针对 0.2 编译的消费者可以链接并运行候选制品 |
| 新 API | 以新增为主，可按功能逐步迁移 |
| 行为兼容 | 保持功能意图，但应用已记录的顺序和生命周期变化 |
| 持久化兼容 | 应用自己负责其 schema 兼容性 |

发布门禁会用同一份冻结的 0.2 Kotlin 调用面分别编译五个既有制品的基线与候选形态，并对每个坐标
执行 archive 级二进制与 Java 源码兼容比较；另有 core-runtime 二进制链接消费者会针对暂存候选版
实际运行。六个发布模块均有受控公开 API 基线。这些是发布要求，不表示尚未发布的 snapshot 已经
成为稳定版。
...
0.3 候选制品以 Java 11 字节码为目标。Android 制品使用 `minSdk 23`，发布工程针对 Android API
36.1 编译；发布和 CI 门禁运行在 JDK 21。候选依赖集合以发布的 Gradle metadata 和 POM 为准，

...
1. 把 `gradle.properties` 中的 `POM_VERSION_NAME` 设为 `X.Y.Z`。
2. 确认 Group 为 `io.github.magic-xu`，且所有必要 POM Metadata 已定稿。
3. 对照当前已检入基线评审有意 API 变化；新制品的首份基线必须显式评审，不能自动接受
   `apiDump` 差异。
4. 确认全部预期模块发布同一版本，且所有内部 Pulse 依赖都使用该版本。
...
正式发布路径是 `.github/workflows/publish-maven-central.yml`：

1. 将 Workflow 配置为只接受准确的稳定 Tag `vX.Y.Z`。
2. 提交已评审的稳定 `X.Y.Z` 候选版本。
3. 在该准确提交上创建 Annotated Tag `vX.Y.Z`。
4. 推送提交和 Tag，不移动或复用已有发布 Tag。
5. 持续观察 `Publish Maven Central` Workflow，直到公共消费者完成。

Workflow 必须只由已配置的准确稳定 Tag 触发，并强制以下顺序：
...
| `release-check` | 验证准确 Tag 与匹配的 `POM_VERSION_NAME`，再运行 `./gradlew verifyMavenCentralConfig mviReleaseCheck --stacktrace` |
| `publish` | 同时依赖前两个 Job，运行 `./gradlew publishAndReleaseToMavenCentral --stacktrace`，等待全部公共发布包，再运行公共消费者 |
...
Workflow 必须轮询 Maven Central 中每个制品的 POM、Gradle Metadata、Source Archive、
Javadoc Archive、Binary 及其签名，再让两个隔离消费者只从 Maven Central 构建。需要本地
复现 `X.Y.Z` 的发布后消费者门禁时运行：
...
全部预期发布包公开且 `publicArtifactSamplesCheck` 通过前，不得宣布可用，也不能移除候选状态。

...
| `mvi-platform-android-testing` | 可选的真实 Split ViewModel 虚拟时间宿主与 Probe |
...
`send(UI)`，以及返回有界 UI-to-executor 路径接纳结果的非阻塞 `trySend(UI)`；Mutation 和 Keyed
Task 只存在于 `PulseIntentContext` 内。

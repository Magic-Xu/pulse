...
- `trySend` 不挂起；邮箱满时返回 `EnqueueResult.Full`。
- `close()` 建立新的输入截止线，`awaitClosed()` 等待截止线前已接纳输入处理完毕。
- `tasks` 提供 `Latest`、`DropWhileRunning`、`Queue`、`Parallel` 和 `Conflate` 五种 keyed task 策略。
...
`PulseRuntimeConfig` 控制邮箱容量、effect 缓冲、dispatcher、时钟、脱敏器和 `PulseErrorHandler`。受控边界内的普通异常会上报为 `PulseFailure`，不会阻断后续输入或其他消费者；`CancellationException` 保持取消语义。
